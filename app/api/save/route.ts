...
type Body = { owners: Owner[]; pets: Pet[] };

export async function POST(req: Request) {
  const body = (await req.json()) as Body;
  const { owners, pets } = body;

  try {
    const ownerIds: string[] = [];

    // Upsert owners, emails, phones
    for (const ow of owners || []) {
      ...
      if (Array.isArray(ow.emails)) {
        ...
        const want = new Set(ow.emails.map((e: string) => e.toLowerCase()));
      ...
      if (Array.isArray(ow.phones)) {
        ...
        const want = new Set(ow.phones.map((e: string) => e));
        const toDelete = existing
    ...
    // Upsert pets + owner links + visits
    for (const p of pets || []) {
      let dbPet = null as any;
      if (p.microchip) {
        dbPet = await prisma.pet.findFirst({
      ...
      // (Ri)crea i link proprietari base (ordinati come da array owners)
    ...
    return NextResponse.json({ ok: true, ownerIds });
  } catch (e: any) {
    console.error(e);
